...
!  tperp=0.0
  ...
  if(nhybrid==0)then
     ...
     do m=1,mi 
        ...
        energy=max(SMALL,0.5*upara*upara+&
...
!hjw open(snapout,file=cdum,status='replace')
         
! time step and grid dimension,write every other point in poloidal direction
!hjw write(snapout,102)real(mstepall+istep)*tstep,q0+0.5*q1+0.25*q2
!hjw write(snapout,101)mbin_u,mbin_psi,mpsi,jm,mzetamax,mpsi/2
     
! write velocity space grids: v_para, pitch, energy to data file
! number of quantities
     i=9
!hjw write(snapout,101)i
!hjw do i=1,mbin_u
!hjw    write(snapout,102)uleft+(uright-uleft)*real(i-1)/real(mbin_u-1)
!hjw enddo
!hjw do i=1,mbin_u
!hjw    write(snapout,102)-1.0+2.0*real(i-1)/real(mbin_u-1)
!hjw enddo
!hjw do i=1,mbin_u
!hjw    write(snapout,102)1.0/eright*real(i-1)/real(mbin_u-1)
!hjw enddo
! velocity space distribution function
!hjw do j=1,mbin_psi
!hjw    write(snapout,102)(ubin(i,j),i=1,mbin_u)
!hjw    write(snapout,102)(dubin(i,j),i=1,mbin_u)
!hjw    write(snapout,102)(pbin(i,j),i=1,mbin_u)
!hjw    write(snapout,102)(dpbin(i,j),i=1,mbin_u)
!hjw    write(snapout,102)(ebin(i,j),i=1,mbin_u)
!hjw    write(snapout,102)(debin(i,j),i=1,mbin_u)
!hjw enddo
...
!hjw write(snapout,101)i
!hjw do i=1,mpsi
!hjw    write(snapout,102)(a0+deltar*real(i))/gyroradius
!hjw enddo
!hjw write(snapout,102)(zonali(i),i=1,mpsi)
!hjw write(snapout,102)(zonale(i),i=1,mpsi)
!hjw write(snapout,102)(phip00(i)/gyroradius,i=1,mpsi)
!     write(snapout,102)(phi00(i)/gyroradius**2,i=1,mpsi)
!hjw write(snapout,102)(marker(i),i=1,mpsi)
!hjw write(snapout,102)(fflows(i),i=1,mpsi)
!hjw write(snapout,102)(dflows(i),i=1,mpsi)
!hjw write(snapout,102)(ftem(i),i=1,mpsi)
!hjw write(snapout,102)(dtem(i),i=1,mpsi)
...
!hjw       write(snapout,102)boozer2x(pdum,tdum)
        enddo
     enddo
     do j=0,jm
        jj=j
        ...
        do i=1,mpsi
           pdum=r2psi(a0+deltar*real(i))
           tdum=pi+2.0*pi*real(jj)/real(jm)
!hjw       write(snapout,102)boozer2z(pdum,tdum)
        enddo
     enddo
...
!hjw       write(snapout,102)(wt*phi(0,igrid(i)+jt)+(1.0-wt)*phi(0,igrid(i)+jt-1))/&
!hjw            gyroradius**2
...
!hjw write(snapout,101)i
     do j=1,jm
        do kz=1,ntoroidal
           do k=1,mzeta
!hjw          write(snapout,102)eachpsi((kz-1)*mzeta*jm+(j-1)*mzeta+k)/gyroradius**2
           enddo
        enddo
     enddo

! eigenmode structure
!hjw write(snapout,101)num_mode,m_poloidal,nmode
     do i=1,mpsi
        do kz=1,num_mode
           do j=1,m_poloidal
!hjw          write(snapout,102)eigenmode(j,kz,i)
           enddo
        enddo
     enddo
