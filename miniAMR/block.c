...
void split_blocks(void)
{
	...
	block *bp, *bp1;
	parent *pp;
	int before_blocks, after_blocks;
		...
		for (n = 0; n < max_active_block; n++)
			if (blocks[n].number >= 0 && blocks[n].level == level) {
				...
				if (bp->refine == 1) {
					...
					//if(my_pe=8){
					//	printf("[%d] n blocks before split : %d \n", my_pe, get_nblocks());
					//}
					//before_blocks = get_nblocks();
					...
					//if(my_pe=8){
					//	printf("[%d] n blocks after split : %d \n",my_pe,  get_nblocks());
					//}
					//after_blocks = get_nblocks();
	...
	//if(my_pe == 8){
	//	printf("[%d] before , after and end of refinement : %d , %d, %d \n", before_blocks, after_blocks, get_nblocks());
	//}
}
